...
export default function PhotoManager({ photos, onChange, userId }) {
  ...
  const addUrls = (urls) => onChange([...photos, ...urls.filter((u) => u && !photos.includes(u))])
  ...
  const onFiles = async (fileList) => {
    ...
    addUrls(results.filter((r) => r.url).map((r) => r.url))
        ...
        <div className="photo-thumbs">
          {photos.map((src, i) => (
            <div className="photo-thumb" key={src + i}>
              <img src={src} alt={'Photo ' + (i + 1)} onError={(e) => { e.currentTarget.parentElement.classList.add('broken') }} />
