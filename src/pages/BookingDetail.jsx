...
export default function BookingDetail() {
  ...
  useEffect(() => {
    if (b && !catalogVenue && !fetchedVenue) fetchVenue(b.venue_id).then((v) => setFetchedVenue(v))
  ...
  useEffect(() => {
    if (b && user && b.user_id === user.id && completed) {
      getReviewForBooking(b.id).then(setMyReview)
    }
  }, [b, user, completed])
        ...
        {images.length > 0 && (
          <Link to={`/venue/${b.venue_id}`} className="booking-photos" aria-label="View venue">
            <div className="booking-photo-main">
              <img src={images[0]} alt={b.venue_name} onError={(e) => { e.currentTarget.style.display = 'none' }} />
            </div>
            {images.length > 1 && (
              <div className="booking-photo-thumbs">
                {images.slice(1, 4).map((src, i) => (
                  <img key={i} src={src} alt={`${b.venue_name} ${i + 2}`} onError={(e) => { e.currentTarget.style.display = 'none' }} />
                ))}
              </div>
            )}
          </Link>
