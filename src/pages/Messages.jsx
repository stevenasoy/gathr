...
const fmtDate = (d) => fmtDateBase(d, 'compact')

export default function Messages({ role = 'guest', embedded = false }) {
  ...
  useEffect(() => {
    ...
    let alive = true
    listMessages(active.col, active.refId).then(({ data }) => { if (alive) setMessages(data || []) })
  ...
  const wrap = (children) => embedded ? children : <><main>{children}</main><Footer /></>
