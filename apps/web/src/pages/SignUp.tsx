        ...
        <div className="max-w-[460px] mx-auto p-8 border border-line rounded-lg bg-white shadow-card">
          {confirm ? (
            <div className="text-center py-5 px-0">
              <h2 className="text-[22px] font-extrabold mb-2">Check your email, {form.name.split(' ')[0]}.</h2>
              <p className="text-ink-soft max-w-[460px] mx-auto mb-5 text-[15px] leading-relaxed">We sent a confirmation link to {form.email}. Click it to activate your account, then sign in.</p>
              <Link to="/signin" state={{ from }} className={baseBtn} style={{ width: 'auto', padding: '13px 22px', display: 'inline-block' }}>Go to sign in</Link>
            </div>
          ) : (
            <>
              <h1 className="text-[26px] font-extrabold mb-2 text-center">Create your account</h1>
              <p className="text-center text-ink-soft text-[14.5px] mb-6">Free forever. No card required.</p>
              ...
              <form onSubmit={onSubmit}>
                <div className="mb-3.5">
                  <label className="block text-[13px] font-bold mb-1.5 text-ink">Full name</label>
                  <input required value={form.name} onChange={onChange('name')} placeholder="Mara R." className={inputCls} />
                </div>
                <div className="mb-3.5">
                ...
                <div className="mb-3.5">
                  ...
                  <input required type="password" minLength={8} value={form.password} onChange={onChange('password')} placeholder="At least 8 characters" className={inputCls} />
                </div>
                ...
                <p className="text-center mt-[18px] text-sm text-ink-soft">Already have an account? <Link to="/signin" state={{ from }} className="text-brand font-semibold hover:underline">Sign in</Link></p>
              </form>
            </>
          )}
        </div>
