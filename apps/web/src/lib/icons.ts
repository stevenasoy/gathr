...
// Keyed by the `icon` names in data/categories.ts
const CATEGORY_ICONS: Record<string, LucideIcon> = {
  Sparkles, Heart, Briefcase, PartyPopper, Presentation, Building2,
  Camera, Trees, Frame, UtensilsCrossed, Waves,
}

// Keyed by amenity label (see AMENITIES in data/categories.ts)
...
export const amenityIcon = (label: string): LucideIcon => AMENITY_ICONS[label] || CheckCircle2